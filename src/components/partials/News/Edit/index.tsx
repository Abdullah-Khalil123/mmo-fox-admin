...
import { useUpdateNews, useNewsById } from '@/hooks/useNews';
...
export default function EditNewsForm({ newsId }: EditNewsFormProps) {
  ...
  const { data: newsData, isLoading: newsLoading } = useNewsById(newsId);
  const games: GameFormData[] = gamesData?.data || [];
  const { mutate, isPending } = useUpdateNews(newsId);
  ...
  useEffect(() => {
    if (newsData?.data && !isFormReady) {
      ...
      reset({
        ...
        type: news.type || 'NEWS',
        gameId: news.gameId ? String(news.gameId) : '',
        published: news.published ?? true,
        isPinned: news.isPinned ?? false,
        tags: news.tags.map((tag: { name: string }) => tag.name) || [],
      });

      // Set tags
      if (news.tags && Array.isArray(news.tags)) {
        setSelectedTags(
          news.tags.map((tag: { name: string }) => tag.name) || []
        );
      }
  ...
  if (newsLoading || gamesLoading || !isFormReady) {
    return (
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                >
                  ...
                  />
                  {imagePreview ? (
                    <div className="relative w-full max-w-2xl mx-auto aspect-video bg-white rounded-lg border shadow-sm overflow-hidden">
                      <Image
                        src={imagePreview}
                        alt="Preview"
                        fill
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Leave empty to keep existing image, or upload a new one to
                  replace it
                </p>
              </div>
            </div>
